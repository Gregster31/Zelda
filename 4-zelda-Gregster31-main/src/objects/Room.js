...
export default class Room {
	...
	updateEntities(dt) {
		this.entities.forEach((entity) => {
			...
			if (entity.didCollideWithEntity(this.player.swordHitbox)) {
				...
				// Create a heart every damage, this works because all enemies only have 1 heart in this game version
	...
	updateObjects(dt) {
		...
		// Keep only the objects (ex: hearts) that were not consumed
	...
	generateObjects() {
		...
		// Randomly places 5 pots in the room
		for(let i = 0; i < 5; i++) {
			objects.push(
				new Pots(
					new Vector(Pots.WIDTH, Pot.HEIGHT),
					new Vector(
						getRandomPositiveInteger(
							Room.LEFT_EDGE + Switch.WIDTH,
							Room.RIGHT_EDGE - Switch.WIDTH * 2
